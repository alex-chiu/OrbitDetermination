function [PMat, NMat, SMat, WMat] = get_ECEF_ECI(JD_UTC, dUT1, xp, yp)
% getECEF2ECI : Get ECEF to ECI (ITRF -> J2000) Matrix
%
% INPUTS
%
...
PMat = euler_rotate(3, PA(1)) * euler_rotate(2, -PA(2)) * euler_rotate(3, PA(3));
PMat = orthodcm(PMat);
end
...
NMat = euler_rotate(1, -meanEps) * euler_rotate(3, dPsi) * euler_rotate(1, trueEps);
NMat = orthodcm(NMat);
end
...
WMat = euler_rotate(1, yp) * euler_rotate(2, xp);
WMat = orthodcm(WMat);
end
